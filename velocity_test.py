...
leg_angle_arr = np.arange(0,31,1)
result = []
initial_steps = 480
sim_steps = 14400
...
for leg_angle in leg_angle_arr:
    ...
    planeId = p.loadURDF("plane.urdf")
    p.changeDynamics(planeId, -1, contactStiffness=1500, contactDamping=30)
    boxId = p.loadURDF(f"same_leg_urdf/backup/{leg_angle}_degree.urdf", cubeStartPos, cubeStartOrientation)
    ...
    for i in range(sim_steps):
        ...
        p.setJointMotorControlArray(boxId, [0, 1, 2, 3], controlMode=mode,
                                    ...
                                    forces=maxforce)
        if i == 2399: # Start calculate after 10 seconds
            PosStart, OrnStart = p.getBasePositionAndOrientation(boxId)
        if (((i + 1) % 2400 == 0) and i > 2400):
            ...
            # velocity[num] = (PosTest[0] - PosStart[0]) / 10
            PosStart = PosTest
            num += 1
        #time.sleep(1. / 240.)
...
df.to_csv("vel_alternate_leg.csv", index=False)
